...
Registering exida’s enterprise applications within a customer’s Microsoft Entra ID tenant enables the customer to centrally manage which users or identities can access exida software, leveraging their existing identity governance policies such as role-based access control and multi-factor authentication. This integration respects the customer’s current identity infrastructure without requiring the exchange of secrets or credentials, and without granting exida any direct access to the customer’s Identity Provider. Authentication is handled securely through standard OpenID Connect and OAuth 2.0 protocols supported by [Microsoft Entra ID](https://learn.microsoft.com/en-us/azure/active-directory/fundamentals/), allowing seamless and secure single sign-on (SSO) experiences. 

::: info

For more on configuring enterprise applications and federated identity, see [Microsoft’s guide to enterprise app integration](https://learn.microsoft.com/en-us/azure/active-directory/manage-apps/what-is-application-management).
...
The script relies on the [Azure CLI](https://learn.microsoft.com/en-us/azure/cloud-shell/overview) for interacting with Microsoft Entra ID. You can use it in PowerShell or your favorite terminal.
...
<TabItem value="apt (Ubuntu, Debian)" label="linux-deb">
